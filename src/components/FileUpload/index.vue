<template>
  <div class="upload-file" :style="{ width: width }">
    ...
    <el-upload
      ref="upload"
      v-loading="uploading"
      :element-loading-text="'文件已上传' + uploadProgress + '%' + (uploadProgress === 100 ? '，上传完成！' : '')"
      ...
      :class="{ 'hide': fileList.length >= limit, 'avatar-uploader': uploadType === 1 && !multiple }"
    >
      ...
      <div slot="file" slot-scope="{ file }" class="pi-w-100P pi-h-100P">
        <div class="el-upload-list__item">
          <img
            ...
            :src="getFileType(file.url) === 'image' ? file.url : getFileTypeImg(file.url) | imageResize"
            alt=""
          />
        </div>
        <div class="el-upload-list__item-actions">
          <div class="pi-abso pi-w-100P pi-h-100P pi-flex-column-center">
            <div>
              ...
              <span v-if="actionBtn" class="el-upload-list__item-remove" @click="handleRemove(file)">
      ...
      <template v-else-if="!showFileList && uploadType === 1 && !multiple">
        <div v-if="fileList.length > 0">
          ...
          <div class="action pi-abso pi-w-100P pi-h-100P">
            <el-button size="mini" round class="preview-btn" type="primary" @click.stop="handlePreview">预览</el-button>
            <el-button v-if="actionBtn" size="mini" round class="restart-btn" type="primary">重新上传</el-button>
          </div>
        </div>
      ...
      <!-- 纵向多文件 -->
      <el-button v-else size="mini" icon="el-icon-plus" type="primary" :disabled="isDisabled">选取文件</el-button>
    ...
    >
      ...
      >
        <div class="pi-align-center pi-flex-sub">
          ...
          <template v-else-if="getFileType(file.url) === 'image'">
            ...
            <div v-if="showFileName" class="pi-flex-sub pi-mg-lr-10 pi-text-ellipsis">
              <el-tooltip class="item" effect="dark" :content="file.name || getFileName(file.url)" placement="top">
        ...
        <div v-if="actionBtn" class="ele-upload-list__item-content-action">
          <el-link :underline="false" icon="el-icon-top" :disabled="index === 0" @click="handleMove(index, 'up')">
            上移
          </el-link>
          <el-link
            :underline="false"
          ...
          <el-link :underline="false" type="danger" icon="el-icon-delete" @click="handleDelete(index)">删除</el-link>
        </div>
      </li>
      <!-- 上传列表 -->
    </transition-group>
...
export default {
  ...
  methods: {
    ...
    getPrivewImages(value) {
      // 找到当前图片的下标
      const index = typeof value === 'object' ? this.fileList.findIndex((item) => item.uid === value.uid) : value
